...
/// <summary>
/// Delta support for a set type's read state.
/// </summary>
public sealed partial class HollowSetTypeReadState
{
    ...
    public void ApplyDelta(HollowBlobInput input, IArraySegmentRecycler memoryRecycler)
    {
        ...
        {
            HollowSetTypeDataElements deltaData = new(memoryRecycler);
            deltaData.ReadDelta(input);

            HollowSetTypeDataElements fromData = _shards[i].DataElements;
            HollowSetTypeDataElements nextData = HollowSetTypeDataElements.ApplyDelta(fromData, deltaData);
