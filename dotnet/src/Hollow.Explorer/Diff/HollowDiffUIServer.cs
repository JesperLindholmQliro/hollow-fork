...
/// <summary>
/// The diff UI on a port of its own, for comparing two blobs without an application to put it in.
/// </summary>
/// <remarks>
/// This is a whole web server, so it is for a developer looking at their own data on their own
/// machine. Inside something already serving requests,
/// <see cref="HollowDiffUIExtensions.AddHollowDiffUI"/> is the way in — and is the one that puts the
/// diff behind whatever authentication that application has.
/// </remarks>
public sealed class HollowDiffUIServer : IAsyncDisposable
{
    private readonly WebApplication _app;

    /// <summary>Serves <paramref name="diff"/> on <paramref name="port"/>.</summary>
    /// <remarks>
    /// The diff must already have been calculated; this shows one, it does not compute one. Calculating
    /// it takes as long as it takes, and doing that while a browser waits would only hide it.
    /// </remarks>
    public HollowDiffUIServer(HollowDiff diff, string fromBlobName, string toBlobName, int port)
        : this(new HollowDiffUI(diff, fromBlobName, toBlobName), port)
    {
    }

    /// <summary>Serves <paramref name="diffUI"/> on <paramref name="port"/>.</summary>
    public HollowDiffUIServer(HollowDiffUI diffUI, int port)
    {
        ...
        // The loopback address rather than the name "localhost", for two reasons: a dev tool showing
        // two whole datasets has no business being reachable from the network, and Kestrel will not
        // take an ephemeral port under the name.
        ...
        builder.Services.AddHollowDiffUI(diffUI);

        _app = builder.Build();
        _app.MapHollowDiffUI();
    }

    /// <summary>The diff being served.</summary>
    public HollowDiffUI DiffUI { get; }
