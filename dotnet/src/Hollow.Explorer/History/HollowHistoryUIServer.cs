...
/// <summary>
/// The history UI on a port of its own, for following a dataset without an application to put it in.
/// </summary>
/// <remarks>
/// This is a whole web server, so it is for a developer watching their own data on their own machine.
/// Inside something already serving requests,
/// <see cref="HollowHistoryUIExtensions.AddHollowHistoryUI"/> is the way in — and is the one that puts
/// the history behind whatever authentication that application has.
/// </remarks>
public sealed class HollowHistoryUIServer : IAsyncDisposable
{
    private readonly WebApplication _app;

    /// <summary>Serves <paramref name="history"/> on <paramref name="port"/>.</summary>
    public HollowHistoryUIServer(HollowHistory history, int port, TimeZoneInfo? timeZone = null)
        : this(new HollowHistoryUI(history, timeZone), port)
    {
    }

    /// <summary>Serves <paramref name="historyUI"/> on <paramref name="port"/>.</summary>
    public HollowHistoryUIServer(HollowHistoryUI historyUI, int port)
    {
        ...
        // whole dataset's history has no business being reachable from the network, and Kestrel will
        // not take an ephemeral port under the name.
        ...
        builder.Services.AddHollowHistoryUI(historyUI);

        _app = builder.Build();
        _app.MapHollowHistoryUI();
    }

    /// <summary>The history being served.</summary>
    public HollowHistoryUI HistoryUI { get; }
